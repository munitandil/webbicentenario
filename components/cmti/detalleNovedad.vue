<template>
  <div>
    <BannerPage
      ...
      :title="
        getDetalleNovedad ? getDetalleNovedad.titulo : 'Consejo Municipal de Teatro Independiente'
      "
...
<script>
import Sidebar from "@/components/cmti/sidebarPost.vue";
import OtrasNovedades from "@/components/cmti/relacionadas.vue";
...
export default {
 ...
 this.$store.dispatch("posts/getSearchDetalle", this.$route.params.id)
  ...
  head() {
    return {
      title: "Cultura Tandil - " + this.titlePage,
      meta: [
  ...
  data() {
    return {
      ...
      sharing: {
       ...
       url: "http://localhost:3000/cmit",
        title: "Cultura Tandil - C.M.T.I",
        description: "Listados de Novedades Cmti",
        quote: "Municipalidad de Tandil",
        hashtags: null,
        twitterUser: "culturaTandil",
      },
      networks: [
        {
          network: "facebook",
  ...
  methods: {
    ...
    f_searchDetalle: function (id) {
      this.$store
        ...
        .catch((error) => {
          //console.log("Error al cargar el detalle cmit: ", error); // undefined
          this.$nuxt.error({
            statusCode: 302,
            message:
              "Ocurrio un error al recuperar el detalle de la novedad cmit id: " +
              id +
              ". Error: " +
              error,
          });
