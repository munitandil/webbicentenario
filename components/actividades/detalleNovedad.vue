<template>
  <div>
    <BannerPage :beadcrumb="beadcrumbPage" :title="
      getDetalleNovedad ? getDetalleNovedad.titulo : 'Detalle de la Novedad'
    " :volanta="getDetalleNovedad ? getDetalleNovedad.volanta : ''"></BannerPage>
    <section>
      <div class="container">
        <div class="row">
          <div class="col-lg-9">
            <div class="dsi-detalle-novedad" v-if="getDetalleNovedad">
              <div class="single_post animation" data-animation="fadeInUp " data-animation-delay="0.1s">
                <div class="dsi-list-image-post-detail mb-md-5 mb-4" v-if="listImages">
                  <no-ssr>
                    <!-- important to add no-ssr-->
                    <carousel v-if="listImages" :autoplay="false" :items="1" :nav="true" :loop="true" :dots="true"
                      :responsive="{
                        0: { items: 1, nav: true },
                        600: { items: 1, nav: true },
                      }">
                      <img :src="item" alt="image222" v-for="item in listImages.split('|#|')" :key="item" />
                    </carousel>
                  </no-ssr>
                </div>
                <div class="blog_content bg-white">
                  ...
                  <div class="dsi-blog-text" id="dsi-detalle-novedad">
                    ...
                    <div class="dsi-texto-novedad" v-if="getDetalleNovedad.texto"
                      v-html="f_escapeHtml(getDetalleNovedad.texto)"></div>
                    <div class="fit-videos" v-if="getDetalleNovedad.video">
                      <iframe width="540" height="304" src="https://www.youtube.com/embed/7e90gBu4pas" frameborder="0"
                        allow="autoplay; encrypted-media" allowfullscreen></iframe>
                    </div>
                    <div class="dsi-descargas-novedades light_gray_bg" v-if="getDetalleNovedad.doc">
                      <div class="comment-title mb-2 mb-sm-4">
                        <h5>Descargas</h5>
                      </div>
                      <ul>
                        <li v-for="item in getDetalleNovedad.doc.split('|#|')" :key="item">
                          <DSILink pCustomLabel=" " :pCustomLink="item" pTipoLink="E" pTitle="Descargar archivo">
                            <i class="icon icon-file-pdf" v-if="item.split('.').pop() === 'pdf'"></i>
                            <i class="icon icon-file-word" v-else-if="item.split('.').pop() === 'doc'"></i>
                            <i class="icon icon-image" v-else-if="
                              item.split('.').pop() === 'png' ||
                              item.split('.').pop() === 'jpeg' ||
                              item.split('.').pop() === 'jpg'
                            "></i>
                    ...
                    <div class="py-4 blog_post_footer">
                      <div class="row justify-content-between align-items-center">
                        <div class="col-md-8 mb-3 mb-md-0">
                          <div class="tags" v-if="getDetalleNovedad.l_tag">
                            <DSILink v-for="item in getDetalleNovedad.l_tag.split(
                              '|#|'
                            )" pCustomLabel=" " :pCustomLink="'/tag?tipo=NOVEDADES&tag=' + item" pTipoLink="I"
                              :key="item.tag" :pTitle="item">
                              {{ item }}
                            </DSILink>
                          </div>
                        </div>
                     <!--   <div class="col-md-4">
                          <ShareNetwork v-for="network in networks" :network="network.network" :key="network.network"
                            ...
                            :twitterUser="sharing.twitterUser" class="dsi-shared-news">
            ...
            <div class="dsi-no-result" v-else>
              <p>No se han encontrado datos.llll</p>
            </div>
          </div>
...
import OtrasNovedades from "@/components/actividades/relacionadas.vue";
...
export default {
 ...
 this.$store.dispatch("posts/getSearchDetalle", this.$route.params.id)
  ...
  data() {
    return {
      ...
      sharing: {
        ...
        title: "Muncinipalidad de Tandil - Actividades",
        description: "Listados de Actividades",
  ...
  computed: {
    ...mapGetters({
      getDetalleNovedad: "posts/getDetalleNovedadAct",
    }),
  },
  mounted() {
  ...
  methods: {
    ...
    f_searchDetalle: function (id) {
      this.$store
        .dispatch("posts/getSearchDetalleAct", id)
        .then((res) => {
          //console.log('data----',res)
          //console.log('this.$route---',window.location.origin + this.$route.fullPath)
          let detalle = res.data.items[0];

          this.titlePage = detalle.titulo;
          this.metaDescription = "Detalle de la Actividad: " + detalle.titulo;
          this.metaKeywords = "";
