<template>
  <div>
    <BannerPage
      ...
      :title="
        getDetalleFeria ? getDetalleFeria.titulo : 'Detalle de la Feria'
      "
      :volanta="getDetalleFeria ? getDetalleFeria.volanta : ''"
    ></BannerPage>
    <section>
      <div class="container">
        <div class="row">
          <div class="col-lg-9">
            <div class="dsi-detalle-novedad" v-if="getDetalleFeria">
              <div
                ...
                <div class="blog_content bg-white">
                  ...
                  <div class="dsi-blog-text" id="dsi-detalle-novedad">
                    <ul class="list_none blog_meta">
                      <li>
                        <a href="#"
                          ...
                          {{ getDetalleFeria.fecha }}</a
                        >
                      </li>
                    </ul>
                    <p>{{ getDetalleFeria.bajada }}</p>
                    <div
                      class="dsi-texto-novedad"
                      v-if="getDetalleFeria.texto"
                      v-html="f_escapeHtml(getDetalleFeria.texto)"
                    ></div>
                    <div class="fit-videos" v-if="getDetalleFeria.video">
                      <iframe
                        width="540"
                        height="304"
                    ...
                    >
                      ...
                      <ul>
                        <li
                          v-for="item in getDetalleFeria.doc.split('|#|')"
                          :key="item"
                        >
                          <DSILink
                            pCustomLabel=" "
                    ...
                    <div class="py-4 blog_post_footer">
                      ...
                      >
                        <div class="col-md-8 mb-3 mb-md-0">
                          <div class="tags" v-if="getDetalleFeria.l_tag">
                            <DSILink
                              v-for="item in getDetalleFeria.l_tag.split(
                                '|#|'
                              )"
                              pCustomLabel=" "
                              :pCustomLink="'/tag?tipo=FERIAS&tag=' + item"
                              pTipoLink="I"
                              :key="item.tag"
                              :pTitle="item"
                            >
              ...
              <OtrasNovedades :pParentId="getDetalleFeria.id" />
            </div>
...
import OtrasNovedades from "@/components/ferias/relacionadas.vue";
...
export default {
 ...
 this.$store.dispatch("posts/getSearchDetalle", this.$route.params.id)
  ...
  data() {
    return {
      ...
      sharing: {
       ...
       url: "http://localhost:3000/ferias",
        title: "Muncinipalidad de Tandil - Ferias",
        description: "Listados de Ferias",
  ...
  computed: {
    ...mapGetters({
           getDetalleFeria: "posts/getDetalleFeria",
    }),
  },
  mounted() {
    let id = this.$route.params.id;
    this.f_searchDetalleFeria(id);
    //this.$fetch()
  },
  ...
  methods: {
    ...
    f_searchDetalleFeria: function (id) {
      this.$store
        .dispatch("posts/getSearchDetalleFeria", id)
        .then((res) => {
          //console.log('data----',res)
          //console.log('this.$route---',window.location.origin + this.$route.fullPath)
          let detalle = res.data.items[0];
          ...
          this.metaDescription = "Detalle de la Feria: " + detalle.titulo;
          this.metaKeywords = "";
